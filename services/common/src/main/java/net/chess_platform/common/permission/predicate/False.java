...
public class False implements QueryCondition {
    ...
    public JPASpecification toJPASpecification() {
        return (path, query, cb) -> cb.isTrue(cb.literal(false));
    }

    @Override
    public Criteria toMongoCriteria() {
        return new Criteria().and("$expr").is(new Document("$eq", List.of(1, 0)));
    }
